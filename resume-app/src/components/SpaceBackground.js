...
const SpaceBackground = () => {
    ...
    useEffect(() => {
        ...
        class Meteor {
            ...
            reset() { this.x = Math.random() * (width * 1.25) - (width * 0.5); this.y = Math.random() * -100 - 10; this.speed = Math.random() * 2 + 0.5; this.length = Math.random() * 250 + 100; this.angle = Math.random() * 0.01 - 0.005 + (- Math.PI * 3 / 2 - Math.atan(width / height)); this.wobbleAngle = Math.random() * Math.PI * 2; this.wobbleFrequency = Math.random() * 0.02 + 0.01; this.wobbleAmplitude = Math.random() * 10 + 2.5; this.life = 1; this.fadeSpeed = Math.random() * 0.005 + 0.001; this.depth = random(0, 30); this.targetDepth = random(240, 1000); }
